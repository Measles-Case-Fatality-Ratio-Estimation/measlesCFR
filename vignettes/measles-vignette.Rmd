...
```{r eval=FALSE}
userSpecifiedHistoricDF <- createHistoricDF(country = 'AFG', bound_sample_range = F, df_incidence = localDF[,c('year','incidence_standardized')], df_mcv1 = localDF[,c('year","mcv1_standardized')], df_maternal_education = localDF[,c('year','maternal_education_standardized')], df_u5mr = localDF[,c('year','u5mr_standardized')], df_prop_urban = localDF[,c('year","prop_urban_standardized')], df_vitA = localDF[,c('year","vitA_standardized')])
```
...
userSpecifiedInputDF <- createInputDF(country = 'AFG',
                                      historic = userSpecifiedHistoricDF, 
...
Run the measlesCFR package as a loop per the following code, revising your inputs and arguments accordingly. This example loops through the above countries for children ages 0 to 4 from years 2000 to 2030, assuming community-based cases given a vaccination program, returning only the mean, upper bound (0.975 percentile), and lower bound (0.025 percentile) estimates.
