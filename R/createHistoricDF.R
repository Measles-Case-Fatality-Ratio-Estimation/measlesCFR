...
#' @param df_incidence Optional parameter. A country-specific numeric vector
#' of measles incidence that is long by year. Values must be non-negative.
#' @param df_mcv1 Optional parameter. A country-specific numeric vector of MCV1
...
#' | ----------------------------------- | -------------------:| ------------------:|
#' | Measles incidence                   | 0.0000              | 0.1096             |
#' | Maternal education                  | 0.2484              | 13.9316            |
#' | MCV1 coverage                       | 0.0019              | 0.99               |
...
#'
#'
#' @return A data frame of numeric class for the historic period (historicDF).
#' The data frame is long by year and wide by covariates.
#' @import data.table
#' @export
createHistoricDF <- function(country, df_incidence=NULL, df_mcv1=NULL,
  ...
  if(!missing(df_mcv1)){
    ...
    if(bound_sample_range == T){
      ...
      message("WARNING: you are possibly including MCV1 coverage values
              outside the range used to fit the modeling object. Proceed
              with caution, or use bound_sample_range = T.")
    }

    df_mcv1$mcv1 <- NULL
    historicDF <- merge(historicDF, df_mcv1, by='year')
  }
  ...
  if(!missing(df_maternal_education)){
    ...
    historicDF <- merge(historicDF, df_maternal_education, by='year')
  }
  ...
  if(!missing(df_u5mr)){
    ...
    if(bound_sample_range == T){
      ...
      message("WARNING: you are possibly including U5MR values outside the
              range used to fit the modeling object. Proceed with caution,
              or use bound_sample_range = T.")
    }

    df_u5mr$u5mr <- NULL
    historicDF <- merge(historicDF, df_u5mr, by='year')
  }
  ...
  if(!missing(df_prop_urban)){
    ...
    historicDF <- merge(historicDF, df_prop_urban, by='year')
  }
  ...
  if(!missing(df_vitA)){
    ...
    df_vitA$vitA <- NULL
    historicDF <- merge(historicDF, df_vitA, by='year')
  }
